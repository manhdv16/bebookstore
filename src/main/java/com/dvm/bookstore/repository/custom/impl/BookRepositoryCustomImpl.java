...
public class BookRepositoryCustomImpl implements BookRepositoryCustom {
     ...
     */
    @Override
    public PageResponse<List<BookDetailResponse>> searchBook(int pageNo, int pageSize, String search, String sort) {
        LOGGER.info("Excute search book with keyword={}", search);
        StringBuilder sqlQuery = new StringBuilder(
                "select new com.dvm.bookstore.dto.response.BookDetailResponse(b.bookId,b.bookName,b.description,b.author,b.image,b.price,b.quantity,b.sold,b.category.categoryName) from Book b");
        ...
        List<Book> books = selectQuery.getResultList();
        List<BookDetailResponse> bookDetailResponses = books.stream()
                .map(book -> new BookDetailResponse(book.getBookId(), book.getBookName(), book.getDescription(), book.getAuthor(), book.getImage(), book.getPrice(), book.getQuantity(), book.getSold(), book.getCategory().getCategoryName()))
                .toList();

        return PageResponse.<List<BookDetailResponse>>builder()
                .page(pageNo)
                .size(pageSize)
                .items(bookDetailResponses).build();
    }

    /**
     * search book with queryDSL
     * @param request
     * @return PageResponse
     */
    @Override
    public PageResponse<BookDetailResponse> searchBookWithQueryDSL(BookSearchRequest request) {
//        QBoo
        return null;
    }
     ...
     * @return
     */
    public PageResponse<List<BookDetailResponse>> searchAdvanceByCriteria(int pageNo, int pageSize, LocalDate cmtDate, String sortBy, String... search) {
        ...
        List<BookDetailResponse> response = books.stream()
                .map(book -> new BookDetailResponse(book.getBookId(),book.getBookName(),book.getDescription(),book.getAuthor(),book.getImage(),book.getPrice(),book.getQuantity(),book.getSold(),book.getCategory().getCategoryName()))
